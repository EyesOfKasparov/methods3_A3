...
pitch_data <- get_study_files(studies="123456") %>% ## NB replace with your path to the files
...
```{r}
pitch_variability <- lmerTest::lmer(pitch_variability ~ Diagnosis + (1|real_ID) + (1|study_nr/trial_nr), model_data)
...
calc_hedge_g <- function(mmodel, group="Diagnosis") {
    ...
    final_data %$% 
        esc_beta(estimate, sd_val, Control, Schizophrenia, es.type = "g")
}
